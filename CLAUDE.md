...
33. **V1 和 V2 精度评测必须使用完全相同的参数**。禁止因 V2 吞吐量低而手动降低 max_tokens、并发数等参数。fast_gpqa.py 自动计算的参数对 V1/V2 统一适用，编排层不得覆盖
34. **operator_search.py 失败时禁止手动重试循环**。如果 operator_search.py 报 NameError/ImportError 等代码级错误（非业务逻辑错误），编排层应立即标记 performance_ok=false 并继续到步骤8，禁止反复重试浪费 token
