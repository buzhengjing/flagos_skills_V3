...
**用途**：仅验证 `vllm_plugin_flaggems` 环境能否正常启动服务并完成推理，不做评测、调优和发布。

**启动方式**：`bash prompts/run_pipeline.sh --mode verify <容器名或镜像地址> <模型名> <模型路径> [GITHUB_TOKEN]`

**用户提供目标 + 模型名 + 模型路径后，①-③ 全自动执行，零交互。任何步骤失败 → 生成 issue → 终止流程。**

```
① 容器准备     → 自动识别容器/镜像 + 使用用户指定的模型路径 + 工具部署
② 环境检测     → inspect_env.py 确认 plugin+flaggems+flagtree 环境
③ 启服务+验证  → default 模式启动 + 健康检查 + curl 推理验证
```

执行顺序：① → ② → ③

### 步骤① 容器准备（复用 flagos-container-preparation）

与完整流程一致：识别容器/镜像 → 模型搜索 → `setup_workspace.sh` 部署工具。

失败处理：记录错误到 trace，终止流程（容器不可用，无法运行 issue_reporter）。

### 步骤② 环境检测（复用 flagos-pre-service-inspection）
...
检查项：
- 确认 `env_type == vllm_plugin_flaggems`
- 确认 `has_flagtree == true`
- 记录核心包版本（torch、vllm、flaggems、flagtree）

环境不符合预期（缺少 flaggems/plugin/flagtree）：
```bash
docker exec $CONTAINER bash -c "PATH=/opt/conda/bin:\$PATH python3 /flagos-workspace/scripts/issue_reporter.py full \
    --type flagtree-error \
    --context-yaml /flagos-workspace/shared/context.yaml \
    ...
    --json"
```
→ 记录到 `logs/issues_startup.log` → 写 trace → 终止流程

### 步骤③ 启服务 + curl 验证

分三个子步骤，任一失败即生成 issue 并终止：

**3a. 启动服务（default 模式）**

不修改 FlagGems 状态，以容器现有 plugin 配置原样启动。plugin 模式环境变量（`USE_FLAGGEMS=1 VLLM_FL_PREFER_ENABLED=true`）由容器自带。

```bash
# 启动服务
docker exec $CONTAINER bash -c "PATH=/opt/conda/bin:\$PATH bash /flagos-workspace/scripts/start_service.sh"
# 等待就绪（指数退避，最长 10 分钟）
docker exec $CONTAINER bash -c "bash /flagos-workspace/scripts/wait_for_service.sh localhost ${PORT} 600"
```
...
docker exec $CONTAINER bash -c "PATH=/opt/conda/bin:\$PATH python3 /flagos-workspace/scripts/issue_reporter.py full \
    --type operator-crash \
    --log-path /flagos-workspace/logs/startup_default.log \
    --context-yaml /flagos-workspace/shared/context.yaml \
    --repo flagos-ai/FlagGems \
    --output-dir /flagos-workspace/results/ \
    --json"
...
```
确认返回 200 且包含模型名。失败 → 同上生成 `operator-crash` issue。

**3c. 推理验证（/v1/chat/completions）**

```bash
curl -s http://localhost:${PORT}/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"model":"<model_name>","messages":[{"role":"user","content":"Hello"}],"max_tokens":32}'
```
确认返回 200 且 `choices` 非空。失败 → 生成 `operator-crash` issue（推理阶段崩溃）。

### verify 模式 Issue 生成规则

| 失败环节 | issue type | 说明 |
|---------|-----------|------|
| 容器准备失败 | 无 | 容器不可用，无法运行 issue_reporter，仅记录 trace |
| 环境缺失 | `flagtree-error` | 缺少 flaggems/plugin/flagtree |
| 服务启动崩溃 | `operator-crash` | 含启动日志 |
| 健康检查失败 | `operator-crash` | 服务启动但 API 不可达 |
| 推理请求失败 | `operator-crash` | 推理阶段崩溃 |

所有 issue 通过 `issue_reporter.py full` 生成 markdown 文件。有 `GITHUB_TOKEN` 时自动提交，无 token 时只保存本地。
...
环境: vllm_plugin_flaggems (flagtree=true)
FlagGems: <version>
FlagTree: <version>

服务启动: ✓
健康检查: ✓ (/v1/models 返回正常)
推理验证: ✓ (/v1/chat/completions 返回正常)
...
结论: 验证失败
失败步骤: ③ 启服务 — 服务启动崩溃
Issue 文件: results/issue_operator-crash_flagos-ai_FlagGems_YYYYMMDD_HHMMSS.md
========================================
```

### verify 模式 Trace 和日志

- 每步写 trace JSON（复用现有格式）：`01_container_preparation.json`、`02_environment_inspection.json`、`03_service_startup.json`
- 失败时追加写入 `logs/issues_startup.log`
- `workflow_ledger` 只包含三个步骤（由 `context.template.yaml` 的 `workflow.mode=verify` 控制）
- `pipeline.log` 格式与完整流程一致，由 `stream_filter.py` 自动生成
